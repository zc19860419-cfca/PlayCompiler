        ...
        try {
            ...
            if (help) {
                ...
                if (script == null) {
                    ...
                    //执行脚本
                    PlayScriptCompilerProxy proxy = new PlayScriptCompilerProxy(new PlayScriptCompiler());

                    AnnotatedTree at = proxy.compile(script, verbose, astDump);

                    if (!at.hasCompilationError()) {
                        Object result = proxy.Execute(at);
